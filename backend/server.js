...
// NEW: Custom test message endpoint with dynamic phone and message
app.post('/api/whatsapp/send-custom-test', async (req, res) => {
  try {
    const { firmId, message, phone } = req.body;
    ...
    console.log(`[Test] Sending custom test message to ${phone} from firm ${firmId}`);
    ...
    console.error('send-custom-test error:', e);
    res.status(500).json({ success: false, message: 'Failed to send test message: ' + e.message });
  }
});

// Legacy test message endpoint (kept for backward compatibility)
