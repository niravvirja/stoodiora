...
import { generateAccountingReportPDF } from '@/components/accounting/AccountingReportPDF';
import { generateFreelancerReportPDF } from '@/components/freelancers/FreelancerReportPDF';
...
import { useGlobalAccountingStats } from './useGlobalAccountingStats';
import { useGlobalFreelancerStats } from './useGlobalFreelancerStats';
...
export const useAccountingExportConfig = (): ExportConfig => {
  const { entries, loading } = useGlobalAccountingStats();
  
  // Remove additionalFilterTypes - category filter is auto-generated by useUniversalExportConfig
  return useUniversalExportConfig({
    entityName: 'accounting',
    title: 'Accounting Report',
    exportFunction: async (data, filterType, filterValue, firmData) => {
      await generateAccountingReportPDF(data, filterType, filterValue, firmData);
    },
    getPreviewData: (data, filterType, selectedValue) => {
      // Apply filter to get accurate count
      const filteredData = filterType && filterType !== 'all' && filterType !== 'global' && selectedValue
        ? applyUniversalFilter(data, filterType, selectedValue)
        : data;
      
      return {
        count: filteredData.length,
        summary: {
          'Total Credits': `₹${filteredData.filter(e => e.entry_type === 'Credit').reduce((sum, e) => sum + e.amount, 0).toLocaleString()}`,
          'Total Debits': `₹${filteredData.filter(e => e.entry_type === 'Debit').reduce((sum, e) => sum + e.amount, 0).toLocaleString()}`,
          'Total Assets': `₹${filteredData.filter(e => e.entry_type === 'Assets').reduce((sum, e) => sum + e.amount, 0).toLocaleString()}`
        }
      };
    }
  });
...
export const useFreelancerExportConfig = (): ExportConfig => {
  ...
  return useUniversalExportConfig({
    ...
    exportFunction: async (data, filterType, filterValue, firmData) => {
      await generateFreelancerReportPDF(data, filterType, filterValue, firmData);
    },
    getPreviewData: (data) => {
      const avgRate = data.length > 0 ? data.reduce((sum, f) => sum + (f.rate || 0), 0) / data.length : 0;
      
      return {
        count: data.length,
        summary: {
          'Total Freelancers': data.length.toString(),
          'Average Rate': `₹${avgRate.toLocaleString()}`,
          'With Email': data.filter((f: any) => f.email).length.toString(),
          'With Phone': data.filter((f: any) => f.phone).length.toString()
        }
      };
    }
  });
...
export const useSalaryExportConfig = (staffData: any[], freelancerData: any[], totalStats: any): ExportConfig => {
  return useMemo(() => {
    ...
    // Add role-based filter type - consolidated roles
    const roleOptions = [
    ...
    // Add staff/freelancer category
    const categoryOptions = [];
    if (staffData.length > 0) categoryOptions.push({ value: 'Staff', label: 'All Staff' });
    if (freelancerData.length > 0) categoryOptions.push({ value: 'Freelancer', label: 'All Freelancers' });
    ...
    return useUniversalExportConfig({
      ...
      exportFunction: async (data, filterType, filterValue) => {
        ...
        if (filterType === 'role_type') {
          ...
          // Handle role-based filters - filter both staff and freelancers by role
          filteredStaffData = staffData.filter(staff => staff.role === filterValue);
          filteredFreelancerData = freelancerData.filter(freelancer => freelancer.role === filterValue);
          
          if (filteredStaffData.length > 0 && filteredFreelancerData.length > 0) {
            reportType = 'all';
          } else if (filteredStaffData.length > 0) {
            reportType = 'staff';
            filteredFreelancerData = [];
          } else if (filteredFreelancerData.length > 0) {
            reportType = 'freelancers';
            filteredStaffData = [];
          }
        }
      ...
      getPreviewData: (data, filterType, selectedValue) => {
        ...
        switch (filterType) {
          case 'role':
            // Role-based filtering - combine staff and freelancers
            const roleStaff = staffData.filter(s => s.role === selectedValue);
            const roleFreelancers = freelancerData.filter(f => f.role === selectedValue);
            const allRoleMembers = [...roleStaff, ...roleFreelancers];
            
            summaryData = {
              'Total Earned': `₹${allRoleMembers.reduce((sum, m) => sum + m.total_earnings, 0).toLocaleString()}`,
              'Total Paid': `₹${allRoleMembers.reduce((sum, m) => sum + m.paid_amount, 0).toLocaleString()}`,
              'Pending': `₹${allRoleMembers.reduce((sum, m) => sum + m.pending_amount, 0).toLocaleString()}`
            };
            count = allRoleMembers.length;
            break;
