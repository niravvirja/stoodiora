      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-1 sm:gap-3 md:gap-4 w-full">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex-1 h-[80px] sm:h-[150px] bg-muted animate-pulse rounded-full" />
          ))}
        </div>
        
        {/* Filter Bar Skeleton */}
        <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
          <div className="h-10 flex-1 bg-muted animate-pulse rounded-2xl" />
    ...
    <div className="space-y-6">
      ...
      {filterState.loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filterState.data.map((quotation) => (
            <QuotationCardGrid
              key={quotation.id}
              quotation={quotation}
