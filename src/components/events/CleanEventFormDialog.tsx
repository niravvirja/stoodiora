    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      if (quotationDetails?.days) {
        ...
        if (dayConfig) {
          ...
          // For drone pilot, ensure we have slots if required
          if (requiredDrone > 0) {
            dayData.drone_pilot_ids = Array.isArray(dayData.drone_pilot_ids) ? dayData.drone_pilot_ids : [];
            while (dayData.drone_pilot_ids.length < requiredDrone) {
        ...
        // For manual events, drone pilot is optional but should be initialized as an array
        dayData.drone_pilot_ids = Array.isArray(dayData.drone_pilot_ids) ? dayData.drone_pilot_ids : [];
      }
  ...
  const handleQuotationSelect = (quotation: Quotation | null) => {
    ...
    if (quotation) {
      ...
      for (let i = 1; i <= totalDays; i++) {
        ...
        dayAssignments.push({
          day: i,
          photographer_ids: Array(Math.max(1, photographerCount)).fill(''),
          cinematographer_ids: Array(Math.max(1, cinematographerCount)).fill(''),
