...
export const ExpenseFormDialog = ({ open, onOpenChange, onExpenseCreated, expense, mode = "create" }: ExpenseFormDialogProps) => {
  ...
  const [pendingValues, setPendingValues] = useState<z.infer<typeof expenseFormSchema> | null>(null);
  ...
  const handleActualSubmit = async (values: z.infer<typeof expenseFormSchema>) => {
      ...
      <UnifiedDialog
        ...
        onConfirm={async () => {
          if (pendingValues) {
            ...
            await handleActualSubmit(pendingValues);
          }
        }}
        variant="info"
        title="Confirm Expense Update"
        description={`Update this expense? Changes will be synced to Google Sheets.`}
