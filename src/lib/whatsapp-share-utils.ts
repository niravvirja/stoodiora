...
export const shareToClientWhatsApp = async (data: WhatsAppShareData) => {
  try {
    ...
    const firmDescription = data.firmData?.description || BUSINESS_DEFAULTS.TAGLINE;
    const contactInfo = data.firmData?.header_left_content || `Contact: ${BUSINESS_DEFAULTS.CONTACT_PHONE}\nEmail: ${BUSINESS_DEFAULTS.CONTACT_EMAIL}`;
